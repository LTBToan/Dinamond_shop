...
export default function Checkout() {
  ...
  const fetchProductInfo = async () => {
    await axios.get(`http://localhost:3344/products/${id}`).then((res) => {
      setCheckoutList([res.data]);
    });
  };

  const fetchOrderedList = async () => {
    await axios.get(`http://localhost:3344/products`).then((res) => {
      setCheckoutList(res.data);
    });
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Title>BILLING</Title>
          <span className={styles.productListSection}></span>
        </Flex>
      </Flex>
      <Footer />
    </>
